    ...
    <div class="p-4 md:p-6 lg:p-8">
        ...
        <div class="mb-8">
            <nav class="text-sm mb-4">
                <ol class="list-none p-0 inline-flex">
                    ...
                    <li class="text-gray-600 dark:text-gray-400">Tambah Data</li>
                </ol>
            </nav>
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white flex items-center">
                        <div class="bg-village-primary/10 p-3 rounded-lg mr-4">
                            <i class="fas fa-plus-circle text-village-primary text-xl"></i>
                        </div>
                        Tambah Data APBDES
                    </h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Tambah data anggaran pendapatan dan belanja desa</p>
                </div>
        ...
        <div class="bg-white dark:bg-dark-300 rounded-xl shadow-lg border border-gray-200 dark:border-dark-400 overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-village-primary to-village-secondary px-6 py-4 border-b border-gray-200 dark:border-dark-400">
                ...
                <p class="text-white/80 text-sm mt-1">Lengkapi semua field yang diperlukan</p>
            </div>

            <!-- Form Content -->
            <div class="p-6">
                <form method="POST" action="{{ route('admin.apbdes.store') }}" class="space-y-6">
                    @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="tahun_anggaran" id="tahun_anggaran" required
                                        ...
                                        @for($year = date('Y') + 5; $year >= 2020; $year--)
                                            <option value="{{ $year }}" {{ old('tahun_anggaran') == $year ? 'selected' : '' }}>
                                                {{ $year }}
                                            </option>
                                        @endfor
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="jenis" id="jenis" required onchange="updateKategori()"
                                        ...
                                        <option value="pendapatan" {{ old('jenis') == 'pendapatan' ? 'selected' : '' }}>
                                            Pendapatan
                                        </option>
                                        <option value="belanja" {{ old('jenis') == 'belanja' ? 'selected' : '' }}>
                                            Belanja
                                        </option>
                                        <option value="pembiayaan" {{ old('jenis') == 'pembiayaan' ? 'selected' : '' }}>
                                            Pembiayaan
                                        </option>
                                    </select>
                                    @error('jenis')
                        ...
                        <div class="p-4 space-y-4">
                            <div>
                                ...
                                <textarea name="uraian" id="uraian" rows="3" required placeholder="Masukkan uraian detail anggaran..."
                                          class="w-full px-3 py-2.5 border border-gray-300 dark:border-dark-400 rounded-lg focus:ring-2 focus:ring-village-primary focus:border-transparent dark:bg-dark-300 dark:text-white transition-all duration-200 @error('uraian') border-red-500 ring-2 ring-red-200 @enderror">{{ old('uraian') }}</textarea>
                                @error('uraian')
                                    <p class="text-red-500 text-xs mt-1 flex items-center">
                                        <i class="fas fa-exclamation-circle mr-1"></i>
                                        {{ $message }}
                            ...
                            <div>
                                ...
                                <textarea name="keterangan" id="keterangan" rows="2" placeholder="Keterangan tambahan (opsional)..."
                                          class="w-full px-3 py-2.5 border border-gray-300 dark:border-dark-400 rounded-lg focus:ring-2 focus:ring-village-primary focus:border-transparent dark:bg-dark-300 dark:text-white transition-all duration-200 @error('keterangan') border-red-500 ring-2 ring-red-200 @enderror">{{ old('keterangan') }}</textarea>
                                @error('keterangan')
                                    <p class="text-red-500 text-xs mt-1 flex items-center">
                                        <i class="fas fa-exclamation-circle mr-1"></i>
                                        {{ $message }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize currency format if there's an old value
