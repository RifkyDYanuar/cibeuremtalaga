...
class PengajuanController extends Controller
{
    ...
    {
        // Ambil semua jenis surat tanpa filter is_active karena kolom tidak ada
    ...
    public function store(Request $request)
    {
        ...
        // Handle file upload ke storage/app/images
        if ($request->hasFile('lampiran')) {
            $file = $request->file('lampiran');
            $filename = 'lampiran_' . time() . '_' . $file->getClientOriginalName();
            
            // Simpan ke storage/app/images/lampiran
            $lampiranPath = storage_path('app/images/lampiran');
            if (!file_exists($lampiranPath)) {
                mkdir($lampiranPath, 0755, true);
            }
            
            $file->move($lampiranPath, $filename);
            $dataUtama['lampiran'] = 'images/lampiran/' . $filename;
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('file_surat')) {
            ...
            if ($pengajuan->file_url) {
                $oldFilePath = storage_path('app/' . $pengajuan->file_url);
            ...
            // Simpan ke storage/app/images/surat_jadi
            $suratPath = storage_path('app/images/surat_jadi');
            if (!file_exists($suratPath)) {
                mkdir($suratPath, 0755, true);
            }
            
            $file->move($suratPath, $filename);
            $pengajuan->file_url = 'images/surat_jadi/' . $filename;
        }
    ...
    public function destroy($id)
    {
        ...
        if ($pengajuan->file_url) {
            $filePath = storage_path('app/' . $pengajuan->file_url);
        ...
        if (isset($data['lampiran'])) {
            $lampiranPath = storage_path('app/' . $data['lampiran']);
            if (file_exists($lampiranPath)) {
                unlink($lampiranPath);
            }
        }
